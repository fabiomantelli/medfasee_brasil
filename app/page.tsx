...
export default function Home() {
  ...
  const { measurements, regionData, isRealDataConnected, stats, isInitializing } = usePMUData();
  ...
  // CORREÇÃO: Remover condição de loading que impedia carregamento dos dashboards
  // Os dashboards devem carregar mesmo com problemas de conexão
  console.log('🔍 Page - Loading condition REMOVIDA - dashboards sempre carregam');
    ...
    <div>
      ...
      <LoadingOverlay loading={isLoading} message="Carregando dados do dashboard...">
        <div className="space-y-6">
          {isInitializing ? (
            <div className="bg-blue-50 border-l-4 border-blue-400 text-blue-800 p-4 mb-4 rounded-r-lg shadow-sm">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium">
                    ⏳ Conectando ao webservice das PMUs...
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4">
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm">
                    Não foi possível conectar ao webservice das PMUs - Timeout ou erro de conexão
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0">
            ...
            <div className="flex items-center space-x-2 flex-shrink-0">
              <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${
                isRealDataConnected ? 'bg-green-500' : 'bg-red-500'
              }`}></div>
              <span className="text-xs md:text-sm text-gray-600 dark:text-gray-300">
                {isRealDataConnected ? 'Dados Reais' : 'Desconectado'}
              </span>
            </div>
          </div>
        </div>
